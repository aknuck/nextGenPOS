...
'''
			...
			for item in self.items:
				count += 1
				receipt += ""+str(count)+"   "+item.getName()+(" "*(width-len(""+str(count)+"   "+item.getName()+"$"+str("{0:.2f}".format(item.getSalePrice())))))+"$"+str("{0:.2f}".format(item.getSalePrice()))+"\n"
				if item.getSaleValue() != 0.00:
					receipt += "       SALE ("+str(int(item.getSaleValue()*100))+"%)\n"
				receipt += "       "+item.getNotes()+"\n"

			receipt += ("-"*width)+"\n"
			receipt += "Subtotal"+(" "*(width-len("Subtotal"+"$"+str("{0:.2f}".format(self.getCurrentTotal())))))+"$"+str("{0:.2f}".format(self.getCurrentTotal()))+"\n"
			if self.taxRate != 0:
			...
			receipt += "\nTotal "+(" "*(width-len("Total $"+str("{0:.2f}".format(self.getCurrentTotal()*(1+self.taxRate))))))+"$"+str("{0:.2f}".format(self.getCurrentTotal()*(1+self.taxRate)))+"\n\n"
			receipt += ("-"*width)+"\n"
			receipt += "Payed With:\n\n"
